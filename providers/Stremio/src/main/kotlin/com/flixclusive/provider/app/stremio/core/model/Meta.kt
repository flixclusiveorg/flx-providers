...
import com.flixclusive.provider.app.stremio.core.util.toMs
...
internal data class Meta(
    ...
    @SerialName("moviedb_id") val tmdbId: String? = null,
...
internal data class FetchMetaResponse(
    @SerialName("meta") val media: Meta? = null,
...
internal fun Meta.toMedia(
    ...
    val externalIds = buildMap {
        ...
        if (tmdbId != null) {
            put(MediaIdSource.TMDB, tmdbId)
        }

        if (tvdbId != null) {
            put(MediaIdSource.TVDB, tvdbId)
        }
    }
    ...
    return when (mediaType) {
        MediaType.MOVIE -> {
            ...
            Movie(
                ...
                casts = cast?.fastMap { Cast(it) } ?: emptyList(),
